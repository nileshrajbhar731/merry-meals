...
export default function MemberDashboardPage() {
  ...
  const [mealKm, setMealKm] = useState("99");
  const dayMapping = {
    0: "Sun",
    1: "Mon",
    2: "Tue",
  ...
  const listenStorageChange = () => {
    ...
    if (!JSON.parse(member)?._id) router.push("/login");
  ...
  return (
    <div className="dashboard">
      {/* <Script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyA-oFxXc0q12Wc5OKVA44Q3Mc2NIzqScWU&libraries=places" /> */}
{loader==true && <Loader submit="load"/>}
      ...
      <div className="availableMeals row">
        {AvilableMeals?.length ? (
          AvilableMeals?.map((e) => {
            // const partnerAddress = e?.mealId?.partnerId?.address;
            // const memberAddress = memberStore?.address;
            //           if (partnerAddress && memberAddress) {

            //             var distanceService = new google.maps.DistanceMatrixService();
            // distanceService.getDistanceMatrix(
            //   {
            //   origins: [memberAddress],
            ...
            //       {/* console.log(response.rows[0].elements[0].distance.text.toLowerCase().replace(" km", "")); */}
            //       setMealKm(response.rows[0].elements[0].distance.text.toLowerCase().replace(" km", ""))
            //       {/* console.log(mealKm) */}
            ...
            return (
              <div key={e._id}>
                <Card
                  ...
                  showMealType={true}
                  // btnTitle="Select Meal"
                  // btnFn={() => alert("activate fn")}
                  // btnFn={() => Meals(e)}
                />
              </div>
            );
